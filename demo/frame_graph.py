...
if __name__ == '__main__':
    ...
    for index, row in code_df.iterrows():
        ...
        plt.plot_date(date[min_data_index-1:], thigh[min_data_index-1:], '.', color=(0.5,0.0,0.0), markersize=1)
        plt.plot_date(date[min_data_index-1:], tlow[min_data_index-1:], '.', color=(0.0,0.5,0.0), markersize=1)
        ...
        x = []
        y = []
        valid_order = set(order_list)
        for index, row in segment_table.iterrows():
            if row["frame_index"] not in valid_order:
                continue
